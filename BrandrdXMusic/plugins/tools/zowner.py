...
async def help(client: Client, message: Message):
    await message.reply_photo(
        photo=f"https://files.catbox.moe/qotedu.jpg",
        caption=f"""🍁𝐂𝐋𝐈𝐂𝐊🥰𝐁𝐄𝐋𝐎𝐖💝𝐁𝐔𝐓𝐓𝐎𝐍✨𝐓𝐎🙊𝐆𝐄𝐓🌱𝐑𝐄𝐏𝐎🍁""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "ƨσʋяcɛ", url=f"https://evidclue.github.io/music_bot/"
                    )
                ]
            ]
        ),
...
async def clones(client: Client, message: Message):
    await message.reply_photo(
        photo=f"https://files.catbox.moe/ghgtux.jpg",
...
async def add_allbot(client, message):
    ...
    if len(command_parts) != 2:
        await message.reply(
            "**⚠️ ɪɴᴠᴀʟɪᴅ ᴄᴏᴍᴍᴀɴᴅ ғᴏʀᴍᴀᴛ. ᴘʟᴇᴀsᴇ ᴜsᴇ ʟɪᴋᴇ » `/gadd @AdaBeats_bot`**"
        )
        return
    ...
    try:
        ...
        async for dialog in userbot.get_dialogs():
            if dialog.chat.id == -1002004045879:
                continue
            try:
